...
func TestShouldReturnCorrectFileHash(t *testing.T) {
	checksumSomeFile := CalculateCollectiveChecksum([]string{"some_file.pem"})
	checksumTestSomeFile := CalculateCollectiveChecksum([]string{"test/some_file.pem"})
...
func TestShouldReturnEmptyFileHashWhenNoPathsPassed(t *testing.T) {
	checksum := CalculateCollectiveChecksum([]string{})
